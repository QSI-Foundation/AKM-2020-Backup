...
namespace AKMLogic
{
	...
	public class AkmRelationship : IDisposable
	{
		...
		private readonly int _initialNodeCount;
		private int _currentNodeCount;

		/// <summary>
		/// Flag indicating if every node in this Relationship is active
		/// </summary>
		public bool IsActive => _initialNodeCount == _currentNodeCount;
		...
		public AkmRelationship(ILogger logger, ICLibCalls cLibCaller, IKeyFactory keyFactory, IKey[] initialKeys, ref AkmConfiguration config)
		{
			...
			_initialNodeCount = config.cfgParams.N;
			_currentNodeCount = config.cfgParams.N;
		}
		...
		internal AkmConfigParams GetCurrentAKMConfig(out byte[] pdv, out byte[] nodeAddresses, out byte[] selfNodeAddr)
		{
			...
			lock (_procLock)
			{
				...
				_currentNodeCount = akmConfig.cfgParams.N;
			}
