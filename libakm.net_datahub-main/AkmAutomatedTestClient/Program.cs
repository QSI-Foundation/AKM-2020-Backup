...
namespace AkmAutomatedTestClient
{
	class Program
	{
		private const int COMMUNICATION_PORT = 8042;

		private static Microsoft.Extensions.Logging.ILogger _logger;

		private static IDictionary<int, int> _frameCounters;
		...
		static void Main(string[] args)
		{
			...
			_frameCounters = new Dictionary<int, int>();
		...
		private static void Receiver_DataReceived(object sender, AkmDataReceivedEventArgs e)
		{
			if ((e?.FrameData?.Length ?? 0) > 0)
			{
				...
				{					
					...
					if (_frameCounters.ContainsKey(e.SenderAddress))
					{
						if (_frameCounters[e.SenderAddress] + 1 != frameNumber)
						{
							_logger.LogError($"Frame Counter error from Node {e.SenderAddress}. Expected {_frameCounters[e.SenderAddress] + 1 }, Received {frameNumber}.");
							_frameCounters[e.SenderAddress] = frameNumber;
						}
						else
						{
							_frameCounters[e.SenderAddress]++;
						}
					}
					else
					{
						if (frameNumber == 1)
							_frameCounters.Add(e.SenderAddress, 1);
						else
						{
							_logger.LogError($"Frame's RelationshipId is out of registered scope {e.RelationshipId}");
							return;
						}
					}

					_logger.LogDebug($"Decrypted Frame data {e.SenderAddress}: {frameNumber} - {message}");
				}
			}
			else
			{
