...
namespace mlibakm
{
    ...
    public class Relationship : IDisposable
    {
        ...
        [DllImport("libakm.dll", CallingConvention = CallingConvention.Cdecl)]
        private static extern void AKMGetConfig(IntPtr relationship, ref Configuration config);
        ...
        public void GetConfig(out ConfigParams akmParams, out byte[] pdv, out byte[] nodeAddresses, out byte[] selfNodeAddr)
        {
            lock (procLock)
            {
                Configuration config = new Configuration();
                AKMGetConfig(relationship, ref config);
                int addrListLen = config.cfgParams.N * config.cfgParams.SRNA;
                akmParams = config.cfgParams;
                pdv = new byte[ParameterDataVectorSize];
                nodeAddresses = new byte[addrListLen];
                selfNodeAddr = new byte[config.cfgParams.SRNA];
                ...
                config.pdv = hPdv.AddrOfPinnedObject();
                config.nodeAddresses = hNodeAddresses.AddrOfPinnedObject();
                config.selfNodeAddress = hSelfNodeAddr.AddrOfPinnedObject();
                AKMGetConfig(relationship, ref config);
                hPdv.Free();
                hNodeAddresses.Free();
                hSelfNodeAddr.Free();
        ...
        public Status LocalSEI()
        {
            ProcessCtx ctx = PrepareProcessCtx();
            ctx.akmEvent = Event.LocalSEI;
            IDecryptedFrame dummy = null;
            ...
            while (true)
            {
                ...
                switch (ctx.cmd.opcode)
                {
                    ...
                    case CmdOpcode.UseKeys:
                        encKeyIdx = ctx.cmd.p1;
                        decKeyIdx = ctx.cmd.p2;
                        break;
